...
4. **Adjacent (bot repo):** as of 8/31 — Tactical Trading Watchdog LastResult=1 *[corrected 2026-09-01: exit 1 is that watchdog's alert-sent path, by design — it detected and Pushover-alerted the 8/31 miss same-day; not a failure]*, `last_successful_run.json` stale since 8/28 *[root cause: machine slept through the 2:30 PM wake task]*, and **the kill switch has entries blocked** ("5 losses in a row, last winner 2026-06-05"). Needs its own session. *[Diagnosis completed read-only 2026-09-01 — see DEV_PLAN.md Phase −1 findings; residual: kill-switch reset decision, wake-timer check.]*
...
1. **Reprioritize: bot health is Phase −1, ahead of everything here.** The kill switch has blocked entries since ~June 5 *[corrected 2026-09-01: since **2026-08-03** — "last winner 2026-06-05" is the last winner's date, not the trip date; the 5th consecutive loss closed 8/3 per trades.jsonl, and entries flowed until then (TQQQ 7/27, XLV 8/3). See DEV_PLAN.md record correction]*, the bot's own watchdog is exiting 1, state is stale since 8/28, and five positions are open on ~$102.7k equity (near Alpaca's $100k paper default, so presumably paper — **verify**). The briefing is polish; that is live risk. It was buried as "adjacent" — wrong altitude.
...
The ecosystem's binding constraint is no longer signal generation — the bot took that over with better-researched strategies. The binding constraint is **oversight**: an autonomous bot trading (presumably paper, eventually maybe real) money, watched by a human who checks in sporadically. The proof case is already on the record: the kill switch tripped ~June 5 and was discovered August 31 — **an 87-day detection latency on the single most important state change in the system.** *[Corrected 2026-09-01: the trip date was 2026-08-03 (see Part IV note); real latency ~28 days. Still a month of silent darkness on the most important state change — the thesis stands, the headline number was rule-7's cousin: a date carried forward without checking what it dated.]* A tool that allocates Ian's scarcest resource (attention) to the system's scarcest input (timely human awareness) is the right thesis. The feedback loop is fast, the contracts exist, the delivery slot precedes the bot's 14:45 entry window by 8 hours.
