...
- **Residual, genuinely Ian's:** (a) ~~the kill-switch reset decision~~ *resolved — live files on 9/2 show `entry_ok: true` and entries resumed 9/1 (presumably the bot-repo session); the pending-queue machinery below keeps the general case*; (b) ~~check Windows wake-timer/power settings~~ *resolved 9/3 — root cause was hibernate-after 3h (S4 unwakeable on this hardware), not wake timers; hibernate disabled on AC; bot fable_plan §10*; (c) ~~TQQQ resting-stop discussion~~ *resolved 9/3 — Ian: accept & document; daily-close trail stays while paper, re-opens at the real-money graduation gate; bot fable_plan §10*.
